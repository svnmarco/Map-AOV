...
@property (assign, nonatomic) BOOL isDisplayingFPS;
...
+ (instancetype)shareFPSDisplay {
	...
	shareDisplay.isDisplayingFPS = YES;
    });
...
- (void)toggleDisplayFPS {
    self.isDisplayingFPS = !self.isDisplayingFPS;
    self.displayLabel.hidden = !self.isDisplayingFPS;
}
...
- (void)initDisplayLabel {
    ...
    _font = [UIFont fontWithName:@"Menlo" size:28];//14
    if (_font) {
        _subFont = [UIFont fontWithName:@"Menlo" size:8];//4
    } else {
        _font = [UIFont fontWithName:@"Courier" size:28];//14
        _subFont = [UIFont fontWithName:@"Courier" size:8];//4
    }
...
double batLeft = (float)[myDevice batteryLevel] * 100;
    ...
    self.displayLabel.font = [UIFont italicSystemFontOfSize:9];



NSString *WMText = [NSString stringWithFormat:@" %d FPS | %@ | Pin: %0.0f  Hello World ",(int)round(fps),mustr,batLeft]; 
