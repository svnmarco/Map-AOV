...
- (void)fetchData:(Lvliangliangwudifangfeng)block
{
    ...
    long GWorld = Read_long(mh_addr + 0x08134688);
    if(!GWorld) return;
    ...
    long juzhen = Read_long(mh_addr + 0x07C6FD38);
    if(!juzhen) return;
    ...
    static int MyTeam,MyPlayerID;
    static Vector myzb = {0};
    ...
    {
        MyObject = Character;
        MyTeam = Read_Int(Character + 0x8A0);
        MyPlayerID = Read_Int(Character + 0x18);
        long rootComponent = Read_long(Controller+0x1a8);
        myzb = Read_Vector(rootComponent + 0x1D0);
    }
    
    long SelfObject = 0;
    for (int i = 0; i < count; i++) {
        long Object = Read_long(Actors + i * 8);
        if (Object <= 0x100000000)  continue;

        int ObjID = Read_Int(Object+0x18);
        if (ObjID != MyPlayerID || ObjID == 0)    continue;
        int Team = Read_Int(Object+0x8A0);
        if (Team == MyTeam || Team == 0) continue;
        

        long rootComponent = Read_long(Object+0x1a8);
        Vector zb = Read_Vector(rootComponent + 0x1d0);
        
        float xue = Read_Float(Object + 0xC28);
